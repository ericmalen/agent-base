// skills-cli tests: `agent-base skills list|add|remove` manage the opt-in
...
test('skills list shows all optionals as available when none selected', () => {
  ...
  try {
    ...
    for (const name of ['retro', 'log-report', 'eval-runner', 'tracker-sync']) {
...
test('skills add installs the skill and records it in the marker', () => {
  const dir = setUpProject();
  try {
    const r = run(['add', 'retro', dir]);
    assert.equal(r.status, 0, r.stderr);
    assert.ok(existsSync(join(dir, '.claude/skills/retro/SKILL.md')), 'skill copied');
    assert.deepEqual(readMarker(dir).optionalSkills, ['retro']);
    // list now reflects it as installed
    const l = run(['list', dir]);
    assert.match(l.stdout, /\[installed\]\s+retro/);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
...
test('skills remove deletes the skill and drops it from the marker', () => {
  ...
  try {
    ...
    const r = run(['remove', 'retro', dir]);
    assert.equal(r.status, 0, r.stderr);
    assert.ok(!existsSync(join(dir, '.claude/skills/retro')), 'skill dir removed');
...
test('skills add on a project without a marker exits 2', () => {
  ...
  try {
    const r = run(['add', 'retro', dir]);
    assert.equal(r.status, 2);
    assert.match(r.stderr, /no agent-base marker/);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
